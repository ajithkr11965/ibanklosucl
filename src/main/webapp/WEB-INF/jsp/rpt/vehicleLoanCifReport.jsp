...
<head>
    <meta charset="UTF-8">
    <title>Vehicle Loan CIF Report</title>
                                    ...
                                    <div class="table-responsive">
                                        <table class="table align-middle table-row-dashed fs-8 gy-3"
                                            ...
                                            <tr>
                                                ...
                                                <td><%=item.get("APPL_NAME")%></td>
                                                <td><%=item.get("REGION")%></td>
                                                <td><%=item.get("CIF_CREATED")%></td>
                                                <td><%=formatDate(item.get("STATUS_DATE"))%></td>
                                                <td><%=getStatusBadge(item.get("STATUS"))%></td>
                                                <td><%=item.get("CIF_ID") == null ? "-" : item.get("CIF_ID")%></td>
                                                <td><%=item.get("TASK_TYPE")%></td>
                                                <td><%=item.get("APPLICANT_ID")%></td>
                                                <td><%=item.get("TASK_ID")%></td>
                                                <td><%=formatDate(item.get("CREATED_DATE"))%></td>
                ...
                buttons: [
                    {
                        ...
                        filename: function () {
                            return 'vehicle_loan_cif_report_' + new Date().toISOString().slice(0, 19).replace(/:/g, "-");
                        }
                    },
                    {
                        extend: 'csv',
