    ...
    <div class="kt">
      <div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-0 ms-3 mb-3 mt-1">
        <div class="d-flex align-items-center mt-2 mb-2">
          ...
          <div data-kt-countup="true" data-kt-countup-value="15000" data-kt-countup-prefix="$" data-kt-initialized="1" class="fs-8  fw-bold counted ms-2">Live Report with DK Score and Bureau Score</div>
        </div>
      </div>
    </div>
                    ...
                    <form name="wicalist" id="wicalist" method="post" action="#">
                      ...
                      <table class="table align-middle table-row-dashed fs-8 gy-3" id="branch-maker-queue-details-table">
                        ...
                        <tr class="text-start text-gray-400 fw-bold fs-8 text-uppercase gs-0">
                          ...
                          <th>REG_NAME</th>
                          <th>LOAN_PROGRAM</th>
                          <th>DATE_OF_DK_RUN</th>
                          <th>DK_SCORE</th>
                          <th>DK_RED_SLAB</th>
                          <th>DK_STATUS</th>
                          <th>BUREAU_SCORE</th>
                          <th>BUREAU_RED_SLAB</th>
                          <th>BUREAU_STATUS</th>
                          <th>CURRENTTAB</th>
                        </tr>
                        </thead>
                        ...
                        <%
                          String temp="";
                          String dk_slab="";
                          String bureau_red_slab="";

                          List<Map<String,String>> report = (List<Map<String,String>>) request.getAttribute("report");
                          for (Map<String,String> item : report) {
                            String wi=item.get("WI_NUM");
                            if(!temp.equals(wi)){
                              temp=wi;
                              dk_slab=item.get("DK_RED_SLAB");
                              bureau_red_slab=item.get("BUREAU_RED_SLAB");
                            }
                        ...
                        <tr>
                          ...
                          <td><%=item.get("CMDATE")%></td>
                          <td><%=item.get("QUEUE")%></td>
                          <td><%=item.get("APPTYPE")%></td>
                          <td><%=item.get("APPLICANT_ID")%></td>
                          <td><%=item.get("SOL_ID")%></td>
                          <td><%=item.get("BR_NAME")%></td>
                          <td><%=item.get("REG_CODE")%></td>
                          <td><%=item.get("REG_NAME")%></td>
                          <td><%=item.get("LOAN_PROGRAM")==null?"-":item.get("LOAN_PROGRAM")%></td>
                          <td><%=item.get("DK_DATE")==null?"-":item.get("DK_DATE")%></td>
                          <td><%=item.get("DK_SCORE")==null?"-":item.get("DK_SCORE")%></td>
                          <td><%=dk_slab==null?"-":dk_slab%></td>
                          <td><%=isPass(dk_slab,item.get("DK_SCORE"))%></td>
                          <td><%=item.get("BUREAU_SCORE")==null?"-":item.get("BUREAU_SCORE")%></td>
                          <td><%=bureau_red_slab==null?"-":bureau_red_slab%></td>
                          <td><%=isPass(bureau_red_slab,item.get("BUREAU_SCORE"))%></td>
                          <td><%=item.get("CURRENTTAB")==null?"-":item.get("CURRENTTAB")%></td>
                        </tr>
                        <% } %>
                        </tbody>
...
<%!
public String isPass(String slab,String score){
  if(!isEmpty(slab) && !isEmpty(score)){
    String slabArr[]=slab.split("-");
    int slabStart=Integer.parseInt(slabArr[0]);
    int slabEnd=Integer.parseInt(slabArr[1]);
    if(Integer.parseInt(score)<=slabEnd && Integer.parseInt(score)>=slabStart){
