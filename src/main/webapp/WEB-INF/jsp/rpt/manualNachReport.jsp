                                ...
                                <div class="card-body py-3">
                                    <div class="table-responsive">
                                        ...
                                        <th>Tenor</th>
                                                ...
                                                <%
                                                    List<Map<String,String>> manualNachReport = (List<Map<String,String>>) request.getAttribute("report");
                                                    for (Map<String,String> item : manualNachReport) {
                                                %>
                                                 <tr>
                                        <td></td>
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="ms-5">
                                                    <a href="#" class="text-gray-800 text-hover-primary fs-8 fw-bold">
                                                     ...
                                                     <td class="account-number"><%=item.get("ACC_NUMBER")%></td>
                                                     <td><%=item.get("ACC_OPEN_DATE")%>
                                                     </td>
                                                     <td><%=item.get("SOL_ID")%> - <%=item.get("BR_NAME")%>
                                                     </td>
                                                     <td><%=item.get("BANK_NAME")%>
                                                     </td>
                                                     <td><%=item.get("COLLECTION_AMOUNT")%>
                                                     </td>
                                                     <td><%=item.get("INSTALMENT_START_DATE")%>
                                                     </td>
                                                     <td><%=item.get("TENOR")%>
                                                     </td>
    ...
    var IbankLOSDashboard = function() {
        ...
        var initDatatable = function () {
            datatable = $(table).DataTable({
                ...
                columnDefs: [
                    {
                        targets: 3, // Account Number column (0-indexed)
                        type: 'string',
                        render: function (data, type, row) {
                            if (type === 'export') {
                                return '\u200C' + data; // Zero-width non-joiner
                            }
                            return data;
                        }
                    }
                ...
                buttons: [
                    {
                        ...
                        customize: function (xlsx) {
                            ...
                            // Format the account number column as text
                            $('row', sheet).each(function (index) {
                                if (index > 0) { // Skip header row
                                    var accountCell = $('c[r^="D"]', this); // Column D (Account Number)
                                    if (accountCell.length > 0) {
                                        // Set cell type to string and preserve leading zeros
                                        accountCell.attr('t', 'inlineStr');
                                        var cellValue = accountCell.find('v').text();
                                        accountCell.html('<is><t>' + cellValue + '</t></is>');
                                    }
                                }
                            });
                        }
