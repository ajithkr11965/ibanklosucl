...
<head>
	<meta charset="UTF-8">
	<title>Branch Maker Queue</title>
	<meta charset="utf-8">
		...
		<div class="kt">
			<div class="border border-gray-300 border-dashed rounded min-w-125px py-3 px-4 me-0 ms-3 mb-3 mt-1">
				<div class="d-flex align-items-center mt-2 mb-2">
					...
					<div data-kt-countup="true" data-kt-countup-value="15000" data-kt-countup-prefix="$" data-kt-initialized="1" class="fs-8  fw-bold counted ms-2">Branch Maker Queue</div>
				</div>
			</div>
		</div>
										...
										<form name="wibmlist" id="wibmlist" method="post" action="wicreate">
											...
											<table class="table align-middle table-row-dashed fs-8 gy-3" id="branch-maker-queue-details-table">
												...
												<%
													...
													List<VehicleLoanMasterDTO> entryQueue = (List<VehicleLoanMasterDTO>) request.getAttribute("entryQueue");
													int count = 1;
													for (VehicleLoanMasterDTO item : entryQueue) {
														String lockDetails ="";
												...
												<tr>
													<td>
														...
														<button type="button" name="modify-btn" value="BC"
													...
													<td>
														<div class="d-flex align-items-center">
															...
															<div class="ms-5">
																<a href="#" class="text-gray-800 text-hover-primary fs-8 fw-bold cust_name" data-slno="<%=item.getSlno()%>">
																	<%= item.getCustName() %>
																</a>
															</div>
														</div>
													...
													<td>
														<button type="button" class="bg-danger symbol symbol-40px me-3 border-transparent bmlistdelbtn" data-slno="<%=item.getSlno()%>">
...
<script>
	$(document).ready(function () {
		...
		$('#branch-maker-queue-details-table').on('click','.bmlistdelbtn',function (e) {
		//$('.bmlistdelbtn').on('click', function (e) {
			var slno = $(this).data('slno');
			let msg = "Please note that deleting this WI will send an SMS and email to the customer (for DSA Channel). Do you want to Proceed?";
			confirmmsg(msg).then(function (result) {
				if (result) {
					bmlistDelWI(slno);
				}
			});
		...
		function bmlistDelWI(slno) {
			$.ajax({
				...
				error: function (xhr, status, error) {
					...
					if (err_data && err_data.errorMessage) {
		...
		$('#branch-maker-queue-details-table').on('click','.cust_name',function (e) {
			e.preventDefault();
			e.stopPropagation();
			//showPageLoading();
			showLoader();
