...
<%
    ...
    if (vehicleLoanMaster != null) {
        ...
        if (nachMandate != null) {
            ...
            mandateMode = nachMandate.getMandateMode();
...
<div class="border rounded px-7 py-3 mb-2">
        ...
        <label class="btn btn-outline btn-outline-dashed btn-active-light-primary acdlen justify-content-start p-5 d-flex align-items-center mb-2 <%=accordion_style%>">
            ...
            <span class="d-block fw-semibold text-start">
                ...
                <span class="text-muted fw-semibold fs-6">
                    Send mandate request to the customer (Mandatory for Other Bank Repayment accounts) to setup Auto Debit
                </span>
            </span>
        </label>
    </div>
    ...
    <div id="nachtable" class="fs-6 collapse ps-2" data-bs-parent="#vl_rc_int">
        <% if (!isOwnBank) { %>
        <% if (nachMandate == null || !"Authorized".equals(nachMandate.getStatus())) { %>
        <div class="row mb-4">
    <div class="col-lg-6">
        <div class="card">
            <div class="card-body">
                <div class="d-flex align-items-center mb-3">
                    ...
                    <h3 class="card-title mb-0">NACH Mandate Mode</h3>
                </div>

                <div class="d-flex gap-3">
    <div class="form-check form-check-custom form-check-solid">
        <input class="form-check-input" type="radio" name="mandateMode"
               id="digitalMode" value="DIGITAL"
               <%= "DIGITAL".equals(mandateMode) ? "checked" : "checked" %>
               <%= "Authorized".equals(mandateStatus) ? "disabled" : "" %>>
        <label class="form-check-label" for="digitalMode">
            <div class="fs-6 fw-bold">Digital</div>
            <div class="text-muted fs-7">Process NACH mandate digitally through customer authorization</div>
        </label>
    </div>
...
<%--               <%= "MANUAL".equals(mandateMode) ? "checked" : "" %>--%>
<%--               <%= "Authorized".equals(mandateStatus) ? "disabled" : "" %>--%>
<%--               disabled>--%>
     <input class="form-check-input" type="radio" name="mandateMode"
               id="manualMode" value="MANUAL">
        <label class="form-check-label" for="manualMode">
            <div class="fs-6 fw-bold">Manual</div>
            <div class="text-muted fs-7">Process NACH mandate through physical form submission</div>
        </label>
    </div>
</div>
            </div>
        ...
        <%} else {%>
        <div> No mandate available or Authorized</div>
        <%}%>
    ...
    <button id="kt_button_nach" type="button"
            class="btn btn-sm btn-light-primary nach-button <%= !showNachRequest ? "d-none" : "" %>"
            data-bs-toggle="tooltip" title="Send NACH mandate request to customer">
    ...
    <button id="kt_button_resend_nach" type="button"
            class="btn btn-sm btn-light-linkedin nach-button <%= !showResendNach ? "d-none" : "" %>"
            data-bs-toggle="tooltip" title="Resend NACH details to customer">
    ...
    <button id="kt_button_cancel_mandate" type="button"
            class="btn btn-sm btn-light-danger nach-button <%= !showNachCancel ? "d-none" : "" %>"
            data-bs-toggle="tooltip" title="Cancel current NACH mandate">
        <span class="indicator-label">Cancel Mandate</span>
        <span class="indicator-progress">Please wait...
            <span class="spinner-border spinner-border-sm align-middle ms-2"></span>
        </span>
    </button>
                            <button id="kt_button_manual_upload" type="button" class="btn btn-sm btn-light-primary nach-button manual-mode-button d-none" data-bs-toggle="tooltip"
                                    title="Create manual NACH mandate"><span class="indicator-label"> <i class="ki-duotone ki-document fs-2"> <span class="path1"></span> <span
                                    class="path2"></span> </i> Manual NACH </span> <span class="indicator-progress">Please wait... <span
                        ...
                        <% if (checker) { %>
                            <div class="text-end mt-3">
                                ...
                                <button type="button" id="nachSave" class="btn btn-sm btn-primary nachSave">
                                    Save
