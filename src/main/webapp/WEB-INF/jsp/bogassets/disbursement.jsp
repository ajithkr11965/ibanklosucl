...
<script>
  $(document).ready(function () {
    ...
    $("#disbursebtn").on('click', function (e) {
      ...
      $('#disbursebtn').attr("disabled",true);
      var slno= $('#slno').val();
      var wiNum= $('#winum').val();
      $.ajax({
        ...
        success: function (response) {
          ...
          $('#disbursebtn').attr("disabled",false);
        },
    ...
    $("#statenqbtn").on('click', function (e) {
      ...
      $.ajax({
        ...
        success: function (response) {
          hideLoader();
          if (response.status === 'S') {
            var temp=response.msg;
            var tranid= temp.split(",")[0];
            var disbamt= temp.split(",")[1];
            var msg='The loan is disbursed successfully, tran id:'+tranid;
            notyalt(msg);
            $('#disbursedAmount').val(disbamt);
          }else if (response.status === 'P') {
            errormsg('Request is still pending : '+response.msg);
          } else {
            alertmsg('Failed: ' + response.msg);
          }
          button.removeAttribute("data-kt-indicator");//.attr("disabled",false);
          $('#statenqbtn').attr("disabled",false);
        },
        error: function (xhr, status, error) {
          hideLoader();
          var err_data = xhr.responseJSON;
    ...
    <div class="accordion-header d-flex collapsed " data-bs-toggle="collapse" id="disblink" data-bs-target="#disbursecontent">
      <label class="btn btn-outline btn-outline-dashed btn-active-light-primary acdlen justify-content-start  p-5 d-flex align-items-center mb-2"
        ...
        <span class="d-block fw-semibold text-start">
                    <span class="text-gray-900 fw-bold d-block fs-4">Disbursement to BOG Pool account</span>
              ...
              <div class="card">
                ...
                <div id="" class="table-responsive border-white border-opacity-15 mb-3 ">
                  ...
                  <div  class="card-footer ">
                    ...
                    <div class="row">
                    <div class="col-lg-12 ">
                      <span class="disbdivaccno">Loan Account Number : <%=loanAcctNo%>.</span>
                      <%
                      if(disbursed){
                        %>
                      <span>Disbursement is completed</span>
                      <%
                      }
                      %>
                    </div>
...
<%--                    <small style="color: gray"><i>After clicking the disburse button, please wait for 2 minutes before proceeding with status enquiry</i></small>--%>
                    <!--
                    <div class="row">
                      <div class="col-lg-1">
                      </div>
