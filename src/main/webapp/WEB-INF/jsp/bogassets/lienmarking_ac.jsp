...
<%
    ...
    String workitemNum = "", showFlg = "", accordion_style = "btn-active-light-primary";
    BigDecimal totalFDBalance = BigDecimal.ZERO;

    if (vehicleLoanMaster != null) {
        workitemNum = vehicleLoanMaster.getWiNum();
...
<div class="border rounded px-7 py-3 mb-2">
    <input type="hidden" name="workitemNum" id="workitemNum" value="<%=workitemNum%>" >
    <div class="">
        ...
        <div id="lientable" class="fs-6 collapse ps-2" data-bs-parent="#vl_rc_int">
                            ...
                            <table class="table table-bordered">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td id="lienStatus_<%= fd.getFdaccnum() %>"><%= fd.getLienStatus() != null ? fd.getLienStatus() : "Pending" %></td>
                                    </tr>
                                    <%
                                        }
                                    }
            ...
            <div class="row mt-3">
                ...
                <button type="button" id="markLienBtn" class="btn btn-primary" data-wi-num="<%= workitemNum %>">Mark Lien</button>
            </div>
            </div>
            <% } %>
...
<script>
    $(document).ready(function () {
        setupEventListeners();
        $('#markLienBtn').on('click', function() {
        var wiNum = $(this).data('wi-num');
        showLoader();
            ...
            success: function(response) {
                ...
                if (response.status === 'success') {
                    response.results.forEach(function(result) {
                        $('#lienStatus_' + result.account_number).text(result.status);
                    });
                    $('#markLienBtn').prop('disabled', true);
                } else {
                    alert('Failed to mark lien: ' + response.message);
                }
            },
            error: function() {
                hideLoader();
                alert('An error occurred while marking the lien.');
            }
        });
    });
    });
