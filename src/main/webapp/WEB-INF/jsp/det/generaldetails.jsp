...
<%
  ...
  String lh_solname="",lh_sol="",rah_sol="",rah_solname="";
  ...
  if(applicant!=null)
  {
    ...
    lh_sol= applicant.getLhsol()==null ?"":applicant.getLhsol();
    ...
    lh_solname= applicant.getLhsolname()==null ?"":applicant.getLhsolname();
...
<%--      <span class="fw-semibold">Note: Once saved, the above fields cannot be edited. Please review all information carefully before submitting.</span>--%>
  ...
  <%if(userdt.isLhUser()){%>
      <div class="row  mb-2">
        <label class="col-form-label col-lg-5">SELECT BRANCH<span class="text-danger">*</span></label>
        <div class="col-lg-7">
          <div class="form-check-horizontal">
            <select class="form-control form-select lh_sol" name="lh_sol">
              <%
                if(lh_sol!=null){
                  out.print("<option value=\""+lh_sol+"\" >"+lh_solname+"("+rsm_sol+")</option>");
                }
              %>
            </select>
          </div>
...
<%--    <button class="btn btn-yellow my-1 me-2 edit-button"><i class="ph-note-pencil  ms-2"></i>Edit</button>--%>
    <button  class="btn btn-primary save-button " data-icon="ph-spinner-gap">Save <i class="ph-paper-plane-tilt ms-2"></i></button>
  </div>
</form>
